...
async def _show_signal(update: Update, context: ContextTypes.DEFAULT_TYPE, symbol: str, display: str):
    ...
    await _send_or_edit(update, text, kb.instrument_actions(symbol, display))
...
async def _run_scan(update: Update, context: ContextTypes.DEFAULT_TYPE, title: str = "\U0001F50D Market Scan"):
    ...
    for display, symbol in config.SCAN_UNIVERSE.items():
        ...
        lines.append(f"{emoji} *{display}* \u2014 {result['signal']} ({result['confidence']}) | RSI {result['rsi']:.0f}")
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting == "watchlist_add":
        ...
        if "error" in result:
            await update.effective_message.reply_text(
                f"\u26a0\ufe0f Couldn't find data for `{symbol}`. Double check the ticker and try again, "
                "or /cancel.",
        ...
        _clear_input_state(context)
        msg = f"\u2b50 Added *{symbol}* to your watchlist." if added else f"*{symbol}* is already in your watchlist."
        await update.effective_message.reply_text(msg, parse_mode=ParseMode.MARKDOWN, reply_markup=kb.watchlist_menu())
        return
    ...
    if awaiting == "journal_add":
        ...
        _clear_input_state(context)
        await update.effective_message.reply_text(
            f"\u2705 Trade logged: *{symbol.upper()}* {side.upper()} {entry} \u2192 {exit_}",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=kb.journal_menu(),
        )
        return
    ...
    for uid in subscribed_ids:
        try:
            ...
            for display, symbol in config.SCAN_UNIVERSE.items():
                ...
                has_signal = True
                emoji = market_data.SIGNAL_EMOJI.get(result["signal"], "\u26aa")
                lines.append(f"{emoji} *{display}* \u2014 {result['signal']} ({result['confidence']})")
