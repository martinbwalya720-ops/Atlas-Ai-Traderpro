...
from config import FOREX_PAIRS, GOLD_SYMBOLS, CRYPTO_PAIRS, TIMEFRAMES, RISK_LEVELS
...
def instrument_actions(symbol: str, display: str) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton("\U0001F504 Refresh Signal", callback_data=f"refresh:{symbol}:{display}")],
        [InlineKeyboardButton("\u2b50 Add to Watchlist", callback_data=f"wladd:{symbol}:{display}")],
        [back_button()],
    ]
    return InlineKeyboardMarkup(rows)
