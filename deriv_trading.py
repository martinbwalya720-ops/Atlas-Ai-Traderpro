...
async def _connect_and_authorize():
    ...
    if resp.get("error"):
        ...
        quote_chars = set('"' + chr(39))
        has_quotes = any(c in quote_chars for c in token)
        diag = (
            "length=" + str(token_len) + ", "
            "starts='" + token_start + "', ends='" + token_end + "', "
            "has_leading_space=" + str(has_leading_space) + ", "
            "has_trailing_space=" + str(has_trailing_space) + ", "
            "has_quotes=" + str(has_quotes)
        )
        msg = resp["error"].get("message", "Authorization failed")
        raise DerivError(msg + " [debug: " + diag + "]")
